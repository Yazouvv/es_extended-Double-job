...
function loadESXPlayer(identifier, playerId, isNew)
    ...
    if not ESX.DoesJobExist(job2, grade2) then
        print(("[^3WARNING^7] Ignoring invalid job2 for ^5%s^7 [job2: ^5%s^7, grade: ^5%s^7]"):format(identifier, job2,
            grade2))
    ...
    local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.inventory,
        userData.weight, userData.job, userData.job2, userData.loadout, GetPlayerName(playerId), userData.coords,
        userData.metadata)
    ...
    AddEventHandler("esx:onPickup", function(pickupId)
        ...
        if pickup then
            ...
            if playerPickupDistance > 5.0 then
                print(("[^3WARNING^7] Player Detected Cheating (Out of range pickup): ^5%s^7"):format(xPlayer
                .getIdentifier()))
                return
            end
